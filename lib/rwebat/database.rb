...
module Rwebat
  module Database
	...
	# Example
    # connect_to_database oracle_db(:host => "localhost", :database => "lavabuild_local", :user => "root", :password => ""), true
    def oracle_db(settings)
      options = {:adapter => "oracle_enhanced"}
      options.merge!(settings)
    end
    ...
    #dbsettting=(:adapter => "sqlserver",:host => "localhost", :database => "lavabuild_local", :user => "root", :password => "")
